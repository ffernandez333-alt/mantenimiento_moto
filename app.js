...
document.addEventListener('click', event => {
  ...
  const updatePlaceholder = () => { const valueLabel = backdrop.querySelector('[data-plan-value-label]'); const isDate = mode.value === 'date'; input.placeholder = 'DD/MM/AA'; input.disabled = !isDate; valueLabel.hidden = !isDate; valueLabel.style.display = isDate ? '' : 'none'; };
  ...
  backdrop.querySelector('[data-plan-save]').addEventListener('click', () => { const value = input.value.trim(); if (mode.value === 'date' && (!value || !/^\d{2}\/\d{2}\/\d{2}$/.test(value))) { window.alert('Indica la fecha con formato DD/MM/AA.'); return; } record.nextChangePlan = { mode: mode.value, value: mode.value === 'date' ? value : '' }; saveComponents(); close(); renderComponents(); });
  backdrop.addEventListener('click', event => { if (event.target === backdrop) close(); });
});
