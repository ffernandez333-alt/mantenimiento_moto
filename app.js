...
function renderComponents() {
  ...
  const nextChange = (item, source) => { const plan = item.nextChangePlan; if (source === 'type') return scheduleLabel(item); if (plan?.mode === 'date') return safeText(plan.value || 'Sin fecha'); if (plan?.mode === 'km') return plan.value ? `${safeText(plan.value)} km` : 'Por estado'; if (plan?.mode === 'state') return 'Por estado'; const field = source === 'marker' ? 'markerHours' : 'realHours'; const linkedEvent = Number.isInteger(item.lastChange?.eventIndex) ? events[item.lastChange.eventIndex] : null; const hours = Number(item.lastChange?.[field] ?? linkedEvent?.[field] ?? (source === 'marker' ? linkedEvent?.hours : linkedEvent?.realHours)); if (!Number.isFinite(hours) || !item.intervalHours) return item.intervalHours ? `${item.intervalHours} h teóricas` : 'Por estado'; return `${Math.round(hours + item.intervalHours).toLocaleString('es-ES')} h`; };
