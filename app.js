...
function renderComponentUsageChart() {
  ...
  if (dataTable) dataTable.innerHTML = maintenanceRows.length ? `<div class="component-usage-table-wrap"><table><thead><tr><th>Fecha</th><th>Mantenimiento</th><th>Horas reales</th><th>Km reales</th><th>Gráfico</th></tr></thead><tbody>${maintenanceRows.map(item => { const hours = realHoursFor(item); const km = realKmFor(item); const valid = Number.isFinite(Number(hours)) && Number.isFinite(Number(km)); return `<tr><td>${safeText(item.date || '')}</td><td>${safeText(item.description || '')}</td><td>${safeText(hours || '—')}</td><td>${safeText(km || '—')}</td><td><span class="chart-data-status ${valid ? 'included' : 'excluded'}">${valid ? 'Incluido' : 'Sin datos reales'}</span></td></tr>`; }).join('')}</tbody></table></div>` : '<p class="component-chart-empty">No hay eventos de mantenimiento registrados.</p>';
