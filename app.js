...
function renderComponentUsageChart() {
  ...
  const points = events.filter(item => item.type === 'Mantenimiento').map(item => ({ item, hours: Number(item.realHours), km: Number(item.realKm) })).filter(point => Number.isFinite(point.hours) && Number.isFinite(point.km) && point.hours >= 0 && point.km >= 0).sort((a, b) => String(a.item.dateISO || '').localeCompare(String(b.item.dateISO || '')));
  const maintenanceRows = events.filter(item => item.type === 'Mantenimiento').sort((a, b) => String(a.dateISO || '').localeCompare(String(b.dateISO || '')));
  const dataTable = document.getElementById('componentUsageDataTable');
  if (dataTable) dataTable.innerHTML = maintenanceRows.length ? `<div class="component-usage-table-wrap"><table><thead><tr><th>Fecha</th><th>Mantenimiento</th><th>Horas reales</th><th>Km reales</th><th>Gráfico</th></tr></thead><tbody>${maintenanceRows.map(item => { const valid = Number.isFinite(Number(item.realHours)) && Number.isFinite(Number(item.realKm)); return `<tr><td>${safeText(item.date || '')}</td><td>${safeText(item.description || '')}</td><td>${safeText(item.realHours || '—')}</td><td>${safeText(item.realKm || '—')}</td><td><span class="chart-data-status ${valid ? 'included' : 'excluded'}">${valid ? 'Incluido' : 'Sin datos reales'}</span></td></tr>`; }).join('')}</tbody></table></div>` : '<p class="component-chart-empty">No hay eventos de mantenimiento registrados.</p>';
  ...
  const maxKm = Math.max(1, ...points.map(point => point.km)); const maxHours = Math.max(1, ...points.map(point => point.hours));
...
document.addEventListener('click', event => {
  ...
  const updatePlaceholder = () => { input.placeholder = mode.value === 'date' ? 'DD/MM/AA' : mode.value === 'hours' ? 'Ej. 640' : 'Ej. 18.000'; backdrop.querySelector('[data-plan-value-label]').hidden = mode.value === 'state'; };
