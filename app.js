...
// Unifica el nombre histórico "Aceite de embrague" con el componente
// catalogado como "Líquido de embrague". También corrige registros ya guardados.
function normalizeComponentName(name) {
  const value = String(name || '').trim();
  if (/^(aceite (?:del|de) embrague)$/i.test(value)) return 'Líquido de embrague';
  if (/^fibra de escape$/i.test(value)) return 'Fibra del escape';
  if (/^tamiz de combustible$/i.test(value)) return 'Filtro de combustible';
  return name;
}
let componentNamesChanged = false;
events = events.map(item => {
  if (!item || item.type !== 'Sustitución de componente') return item;
  const normalize = change => {
    if (!change || normalizeComponentName(change.name) === change.name) return change;
    componentNamesChanged = true;
    return { ...change, name: normalizeComponentName(change.name) };
  };
  const componentChanges = (item.componentChanges || (item.componentChange ? [item.componentChange] : [])).map(normalize);
  const componentChange = normalize(item.componentChange);
  return { ...item, componentChange, componentChanges };
});
if (componentNamesChanged) saveEvents();
const componentCurveAlignmentKey = bikeStorageKey('componentCurveAlignment20260924');
if (!localStorage.getItem(componentCurveAlignmentKey)) {
  const maintenanceReadings = events.filter(item => item?.type === 'Mantenimiento').map(item => ({
    date: Date.parse(`${item.dateISO || ''}T12:00:00`),
    realHours: Number(item.realHours ?? item.hours),
    realKm: Number(item.realKm ?? item.km)
  })).filter(item => Number.isFinite(item.date) && item.realHours > 0 && item.realKm > 0);
  if (maintenanceReadings.length) {
    events = events.map(item => {
      if (item?.type !== 'Sustitución de componente') return item;
      const date = Date.parse(`${item.dateISO || ''}T12:00:00`);
      if (!Number.isFinite(date)) return item;
      const nearest = maintenanceReadings.reduce((best, candidate) => Math.abs(candidate.date - date) < Math.abs(best.date - date) ? candidate : best);
      return { ...item, realHours: nearest.realHours, realKm: nearest.realKm };
    });
    saveEvents();
  }
  localStorage.setItem(componentCurveAlignmentKey, '1');
}
...
document.getElementById('eventForm').addEventListener('submit', async event => {
  ...
  const eventRealHours = Number(editedEvent.realHours);
  const eventRealKm = Number(editedEvent.realKm);
  const eventMarkerHours = Number(editedEvent.hours);
  const eventMarkerKm = Number(editedEvent.km);
  if (Number.isFinite(eventRealHours) && eventRealHours >= Number(bikeData.realHours || 0)) bikeData.realHours = eventRealHours;
  if (Number.isFinite(eventRealKm) && eventRealKm >= Number(bikeData.realKm || 0)) bikeData.realKm = eventRealKm;
  if (Number.isFinite(eventMarkerHours) && eventMarkerHours >= Number(bikeData.markerHours || 0)) bikeData.markerHours = eventMarkerHours;
  if (Number.isFinite(eventMarkerKm) && eventMarkerKm >= Number(bikeData.markerKm || 0)) bikeData.markerKm = eventMarkerKm;
  saveBikeProfiles();
...
const deleteBikeButton = (() => {
  ...
  button.type = 'button'; button.id = 'deleteBike'; button.className = 'danger-button hidden'; button.textContent = 'Eliminar moto';
  document.querySelector('#bikeForm .modal-actions')?.prepend(button);
  return button;
})();
...
function renderComponentUsageChart() {
  ...
  const storedIncluded = localStorage.getItem(bikeStorageKey('componentChartIncluded'));
  const included = new Set(storedIncluded === null ? names : JSON.parse(storedIncluded || '[]'));
  const storedSelection = localStorage.getItem(bikeStorageKey('componentChartSelection'));
  const selected = new Set(storedSelection === null ? [...included] : JSON.parse(storedSelection || '[]'));
  const selectedNames = names.filter(name => included.has(name));
  options.innerHTML = selectedNames.length ? `<div class="component-usage-select-actions"><button type="button" class="quiet-button" data-component-select-all>Seleccionar todo</button><button type="button" class="quiet-button" data-component-select-none>Seleccionar nada</button></div>${selectedNames.map(name => `<label><input type="checkbox" value="${safeText(name)}" ${selected.has(name) ? 'checked' : ''} /> <span>${safeText(name)}</span></label>`).join('')}` : '<small>No hay componentes incluidos. Usa la tabla inferior para añadirlos.</small>';
  options.onclick = event => { const button = event.target.closest('[data-component-select-all],[data-component-select-none]'); if (!button) return; event.preventDefault(); localStorage.setItem(bikeStorageKey('componentChartSelection'), JSON.stringify(button.hasAttribute('data-component-select-all') ? selectedNames : [])); renderComponentUsageChart(); };
  ...
  const componentRows = names.map(name => {
    const records = events.map((item, eventIndex) => ({ item, eventIndex })).filter(({ item }) => item.type === 'Sustitución de componente' && (item.componentChanges || [item.componentChange]).some(change => change?.name === name)).sort((a, b) => String(b.item.dateISO || '').localeCompare(String(a.item.dateISO || '')));
    const latest = records[0];
    return { name, records, latest };
  });
  const dataTable = document.getElementById('componentUsageDataTable');
  if (dataTable) dataTable.innerHTML = componentRows.length ? `<div class="component-usage-table-wrap"><table><thead><tr><th>Incluir</th><th>Componente</th><th>Último cambio</th><th>Horas reales</th><th>Km reales</th></tr></thead><tbody>${componentRows.map(({ name, latest }) => { const item = latest?.item; const hours = item ? realHoursFor(item) : ''; const km = item ? realKmFor(item) : ''; return `<tr><td><input type="checkbox" class="component-chart-toggle" value="${safeText(name)}" ${included.has(name) ? 'checked' : ''} /></td><th scope="row">${safeText(name)}</th><td>${safeText(item?.date || '—')}</td><td>${safeText(hours || '—')}</td><td>${safeText(km || '—')}</td></tr>`; }).join('')}</tbody></table></div>` : '<p class="component-chart-empty">No hay cambios de componentes registrados.</p>';
  dataTable?.querySelectorAll('.component-chart-toggle').forEach(input => input.addEventListener('change', () => { const nextIncluded = new Set(included); const nextSelected = new Set(selected); if (input.checked) { nextIncluded.add(input.value); nextSelected.add(input.value); } else { nextIncluded.delete(input.value); nextSelected.delete(input.value); } localStorage.setItem(bikeStorageKey('componentChartIncluded'), JSON.stringify([...nextIncluded])); localStorage.setItem(bikeStorageKey('componentChartSelection'), JSON.stringify([...nextSelected])); renderComponentUsageChart(); }));
  ...
  const componentPoints = events.filter(item => item.type === 'Sustitución de componente').map(item => ({ item, hours: Number(item.realHours), km: Number(item.realKm) })).filter(point => String(point.item.realHours ?? '').trim() !== '' && String(point.item.realKm ?? '').trim() !== '' && Number.isFinite(point.hours) && Number.isFinite(point.km) && point.hours > 0 && point.km > 0);
...
addBikeButton.addEventListener('click', () => {
  ...
  deleteBikeButton.classList.add('hidden');
...
document.getElementById('editBike').addEventListener('click', () => {
  ...
  document.getElementById('formPhoto').value = '';
  deleteBikeButton.classList.toggle('hidden', bikeProfiles.length <= 1);
  bikeModal.classList.remove('hidden');
});
...
deleteBikeButton.addEventListener('click', () => {
  if (bikeProfiles.length <= 1) return;
  if (!window.confirm(`¿Eliminar ${bikeData.brand} ${bikeData.model}? Se borrarán sus eventos y configuración.`)) return;
  const deletedId = activeBikeId;
  bikeProfiles = bikeProfiles.filter(profile => profile.id !== deletedId);
  localStorage.removeItem(`motoEvents:${deletedId}`);
  localStorage.removeItem(`motoTasks:${deletedId}`);
  Object.keys(localStorage).filter(key => key.includes(`:${deletedId}`)).forEach(key => localStorage.removeItem(key));
  activeBikeId = bikeProfiles[0].id;
...
function applyMaintenanceReadingsToBike(session) {
  ...
  bikeData.markerHours = Math.max(Number(bikeData.markerHours) || 0, markerHours);
  bikeData.markerKm = Math.max(Number(bikeData.markerKm) || 0, markerKm);
  bikeData.realHours = Math.max(Number(bikeData.realHours) || 0, realHours);
  bikeData.realKm = Math.max(Number(bikeData.realKm) || 0, realKm);
  saveBikeProfiles();
  updateBikeView();
}
