...
function renderComponentUsageChart() {
  ...
  chart.onpointermove = event => { if (!dragStart) return; usageChartView.x = dragStart.panX + event.clientX - dragStart.x; usageChartView.y = dragStart.panY + event.clientY - dragStart.y; applyView(); };
  chart.onpointerup = () => { dragStart = null; };
  chart.onpointercancel = () => { dragStart = null; };
  chart.onwheel = event => { event.preventDefault(); usageChartView.scale = Math.max(1, Math.min(4, usageChartView.scale * (event.deltaY < 0 ? 1.1 : 0.9))); applyView(); };
