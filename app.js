...
function renderTimeline() {
  ...
  if (!timeline) return;
  const orderedEvents = events.map((item, index) => ({ item, index })).filter(({ item }) => !item.maintenanceInterval && (filterType === 'all' || item.type === filterType) && (filterYear === 'all' || String(item.dateISO || '').startsWith(filterYear))).sort((a, b) => { const result = String(a.item.dateISO || '').localeCompare(String(b.item.dateISO || '')); return sortDirection === 'desc' ? -result : result; });
